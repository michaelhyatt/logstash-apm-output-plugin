...
public class SuccessTests extends BaseApmTests {
	...
	@Test
	public void testOneTransactionWithException() {
		...
		Event eventSpanEnd = createEventWithResult(EventsProcessor.SPAN_END, "simple-test-1-id", "simple-test-span-name-1",
				timestamp1, EventsProcessor.EXCEPTION);
	...
	private Event createEventWithResult(String command, String id, String name, Timestamp timestamp,
			String result) {
		Event event = createEvent(command, id, name, timestamp);
		
		event.setField(EventsProcessor.APM_RESULT, result);
